...
async def upsert_pw(db: Session, user_id: UUID, pw: str, user_record: int) -> Optional[Password]:
    prev_pw = db.query(Password).filter(Password.user_id ==
                                        user_id, Password.record == user_record).first()
    if prev_pw is None:
        new_pw = Password(
            user_id=user_id,
            record=user_record,
